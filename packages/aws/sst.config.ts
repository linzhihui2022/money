...
export default $config({
  app(input) {
    return {
      ...
      home: "aws",
      providers: { aws: { profile: process.env.AWS_PROFILE, region: "ap-southeast-1" } },
    };
  },
  async run() {
    ...
    const userPoolClient = userPool.addClient("Web", {
      transform: { client: { enableTokenRevocation: true, explicitAuthFlows: ["ALLOW_USER_PASSWORD_AUTH", "ALLOW_REFRESH_TOKEN_AUTH"] } },
    });
    new sst.aws.CognitoIdentityPool("MyIdentityPool", { userPools: [{ userPool: userPool.id, client: userPoolClient.id }] });
    const api = new sst.aws.ApiGatewayV2("api", { cors: false, domain: { name: `${$app.stage}.linzhihui.app` } });
    const auth = api.addAuthorizer({
      name: "auth",
      jwt: { audiences: [userPoolClient.id], issuer: $interpolate`https://cognito-idp.ap-southeast-1.amazonaws.com/${userPool.id}` },
    });
    ...
    api.route("GET /accounts", { handler: "src/account.list", link: [accountDB] }, jwt);
    api.route("GET /account/{id}", { handler: "src/account.get", link: [accountDB] }, jwt);
    api.route("POST /account", { handler: "src/account.add", link: [accountDB] }, jwt);
    api.route("PUT /account/{id}/name", { handler: "src/account.updateName", link: [accountDB] }, jwt);
    api.route("PUT /account/{id}/value", { handler: "src/account.updateValue", link: [accountDB] }, jwt);
    api.route("DELETE /account/{id}", { handler: "src/account.del", link: [accountDB] }, jwt);
    ...
    api.route("PUT /bill/{id}/desc", { handler: "src/bill.updateDesc", link: [billDB] }, jwt);
    api.route("PUT /bill/{id}/value", { handler: "src/bill.updateValue", link: [billDB] }, jwt);
    api.route("PUT /bill/{id}/account", { handler: "src/bill.updateAccount", link: [billDB] }, jwt);
    api.route("PUT /bill/{id}/category", { handler: "src/bill.updateCategory", link: [billDB] }, jwt);
    api.route("PUT /bill/{id}/date", { handler: "src/bill.updateDate", link: [billDB] }, jwt);
    api.route("PUT /bill/{id}/active", { handler: "src/bill.updateActive", link: [billDB] }, jwt);
    api.route("DELETE /bill/{id}", { handler: "src/bill.del", link: [billDB] }, jwt);
