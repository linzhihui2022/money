...
const InlineFormItem = ({
    ...
    return (
        <FormItem className="grid grid-cols-1 items-center gap-x-3 gap-y-1.5">
...
const FormTitle = ({
    ...
    <h3 className={cn("text-center text-xl font-bold", className)} {...props}>
        {children}
    </h3>
)
const SubmitButton = <T extends FieldValues>({
    ...
    return (
        <div className={cn("flex w-full", { "justify-end": position === "end" })}>
            <Button
                className={cn({ "w-full": position === "full" })}
                type="submit"
                disabled={!formState.isDirty || formState.isSubmitting || pending}>
                {formState.isSubmitting || pending ? <LoaderCircle className="animate-spin" /> : <></>}
                {children || t("Submit")}
            </Button>
        </div>
    )
