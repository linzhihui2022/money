...
import { Form, FormField, FormTitle, InlineFormItem, SubmitButton } from "@/components/ui/form"
...
const formSchema = z.object({
    name: z.string().min(1),
    items: z.array(z.object({ quantity: z.coerce.number().min(1), food: z.number() })).min(1),
    content: z.object({
        ...
        steps: z
            .array(
                z.object({
                    ...
                    phase: z.enum([CookbookStepPhase.PROGRESS, CookbookStepPhase.PREPARE, CookbookStepPhase.DONE]),
                    key: z.string().uuid(),
                })
            )
            .min(1),
...
export function UpdateCookbook({
    ...
    async function onSubmit(data: FormFields) {
        startTransition(async () => {
            ...
            const oldItemsMap = cookbook.items.reduce<Record<string, (typeof cookbook)["items"][number]>>(
                (pre, cur) => {
                    pre[cur.food.id] = cur
                    return pre
                },
    ...
    return (
        <Form {...form}>
            <form className="mx-auto max-w-lg space-y-3 py-3" onSubmit={form.handleSubmit(onSubmit)}>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <InlineFormItem label={t("Steps")}>
                                <CookbookContentSteps value={field.value} setValueAction={field.onChange} />
                            </InlineFormItem>
                        )}
                    />
                </AiCookbook>
