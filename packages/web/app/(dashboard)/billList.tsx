...
export const BillList = async (props: { searchParams: Promise<{ category: string; account: string }> }) => {
  ...
  const filters =
    accountChecked.length || categoryChecked.length ? (
      <div className="flex flex-wrap -mb-1 -mr-1 mt-3 px-3">
        {accountChecked
          .map((i) => accounts[i])
          .map((account) => (
            <Badge key={account.id} className="mr-1 mb-1" variant="secondary">
              <Link className="flex items-center space-x-1" href={`/?${queryToggle(query, "account", account.id)}`}>
                <X className="size-3" />
        ...
        {categoryChecked
          ...
          .map((category) => (
            <Badge key={category.id} className="mr-1 mb-1" variant="secondary">
              <Link className="flex items-center space-x-1" href={`/?${queryToggle(query, "category", category.id)}`}>
                <X className="size-3" />
  ...
  return (
    <>
      {filters}
      <div className="grid md:grid-cols-[auto,auto,1fr,auto,auto] gap-y-2 gap-x-4 text-sm px-3">
          ...
          <Fragment key={date}>
            ...
            {bills.map((i, index, arr) => (
              <Fragment key={i.id}>
                ...
                <Cell label="Account">
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive(query, "account", i.account) ? "underline text-accent-foreground" : "",
                    )}
                  >
                ...
                {index < arr.length - 1 && <Separator className="col-span-full" />}
              </Fragment>
            ))}
          </Fragment>
        ))}
