...
export const getPrice = <T extends Record<string, number>>(usage: T, model: keyof typeof priceMap) => {
    const price = Object.entries(priceMap[model]).reduce<number>((pre, [key, price]) => {
        if (key in usage) {
